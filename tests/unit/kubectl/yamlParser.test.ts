...
`
  ...
  describe('parseKubernetesYaml - Init Containers', () => {
    it('should parse Pod with init containers', () => {
      const yaml = `
apiVersion: v1
kind: Pod
metadata:
...
`
      ...
      expect(result.ok).toBe(true)
      if (result.ok && result.value.kind === 'Pod') {
        expect(result.value.spec.initContainers).toBeDefined()
        expect(result.value.spec.initContainers).toHaveLength(1)
        expect(result.value.spec.initContainers?.[0].name).toBe('init-setup')
        expect(result.value.spec.initContainers?.[0].image).toBe('busybox:latest')
        expect(result.value.spec.initContainers?.[0].command).toEqual(['touch'])
        expect(result.value.spec.initContainers?.[0].args).toEqual(['/tmp/ready'])
      }
    })

    it('should parse Pod with multiple init containers', () => {
      const yaml = `
apiVersion: v1
kind: Pod
metadata:
...
`
      ...
      expect(result.ok).toBe(true)
      if (result.ok && result.value.kind === 'Pod') {
        expect(result.value.spec.initContainers).toHaveLength(2)
        expect(result.value.spec.initContainers?.[0].name).toBe('init-1')
        expect(result.value.spec.initContainers?.[1].name).toBe('init-2')
      }
    })

    it('should parse Pod with container command and args', () => {
      const yaml = `
apiVersion: v1
kind: Pod
metadata:
...
`
      ...
      expect(result.ok).toBe(true)
      if (result.ok && result.value.kind === 'Pod') {
        expect(result.value.spec.containers[0].command).toEqual(['sh'])
        expect(result.value.spec.containers[0].args).toEqual(['-c', 'echo hello'])
      }
    })

    it('should fail with invalid init container manifest', () => {
      const yaml = `
apiVersion: v1
kind: Pod
metadata:
